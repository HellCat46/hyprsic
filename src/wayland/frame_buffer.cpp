...
wl_buffer *WaylandFrameBuffer::DrawFrame() {
  ...
  if (fd < 0) {
    std::cerr
        << "[ERROR] Failed to create the File Descriptor for Shared Memory."
        << std::endl;
    err = 1;
    return nullptr;
  }
  ...
  try {
    ...
    std::cerr
        << "[ERROR] Failed to Truncate the File Descriptor for Shared Memory."
        << std::endl;
    err = 1;
    return nullptr;
  }
  uint8_t *poolData = static_cast<uint8_t *>(
      mmap(nullptr, size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0));

  shm_pool = wl_shm_create_pool(shm, fd, size);

  buff = wl_shm_pool_create_buffer(shm_pool, 0, width, height, stride,
  ...
  for (int y = 0; y < height; ++y) {
    for (int x = 0; x < width; ++x) {
      if ((x >= 100 && x <= 120 && y >= 100 && y <= 980) ||
          (y >= 100 && y <= 120 && x >= 100 && x <= 1820) ||
          (x >= 1800 && x <= 1820 && y >= 100 && y <= 980) ||
          (y >= 960 && y <= 980 && x >= 100 && x <= 1820)) {
